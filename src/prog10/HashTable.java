...
public class HashTable<K, V> extends AbstractMap<K, V> {
  ...
  public V remove (Object key) {
    ...
    // What do you do if the first element o f the list has the key?
    // Rehash to half the current length if size is 1/4 the length or less.

    Entry entry = table[index];

    if(entry.key.equals(key))  // first entry in the linked list
    {
   ...
   // NOT FIRST INDEX IN LINKED LIST



      while(entry.next!=null){
        if(entry.next.key.equals(key)){
          V val = entry.next.value;
